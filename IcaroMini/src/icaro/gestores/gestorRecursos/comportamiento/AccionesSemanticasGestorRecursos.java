...
public class AccionesSemanticasGestorRecursos extends
	...
	public void listarRecursos() {
		try {
			 logger.debug("GestorRecursos: Listando los recursos definidos en la configuraci�n.");
			trazas.aceptaNuevaTraza(new InfoTraza("GestorRecursos",
			...
			e.printStackTrace();
			logger.error("GestorRecursos: Hubo problemas al listar los recursos desde la configuraci�n.");
			trazas.aceptaNuevaTraza(new InfoTraza("GestorRecursos",
		...
		while (iterador.hasNext() && (encontrado == false)) {
			...
			if (j == indice.intValue()) {// es el que debemos arrancar
				...
				try {
					...
					logger.error("GestorRecursos: Hubo problemas al crear el recurso "+ nombre+ " desde la configuraci�n. Recuperando errores de creaci�n.");
					trazas.aceptaNuevaTraza(new InfoTraza("GestorRecursos",
							"Hubo problemas al crear el recurso "+ nombre+ " desde la configuraci�n. Recuperando errores de creaci�n.",
							InfoTraza.NivelTraza.error));
					ex.printStackTrace();
					error = true;
				}
	...
	private void crearUnRecurso(DescInstanciaRecursoAplicacion recurso) throws Exception {
		try {
			...
			trazas.aceptaNuevaTraza(new InfoTraza("GestorRecursos",
					...
					InfoTraza.NivelTraza.debug));
			// Recurso de aplicaci�n: local o remoto?
	...
	public void reintentarCreacionRecurso(List<DescInstanciaRecursoAplicacion> lista, DescInstanciaRecursoAplicacion recurso,
		...
		if (!fin_reintentos) { // se reintenta la creacion
			...
			try {
				...
				logger.debug("GestorRecursos: A�adiendo recurso " + idRecurso	+ " a la lista de objetos gestionados.");
				trazas.aceptaNuevaTraza(new InfoTraza("GestorRecursos",
						"A�adiendo recurso " + idRecurso	+ " a la lista de objetos gestionados.",
						InfoTraza.NivelTraza.debug));
				...
				trazas.aceptaNuevaTraza(new InfoTraza("GestorRecursos",
						"Hubo problemas al crear el recurso "+ idRecurso+ " desde la configuraci�n. Recuperando errores de creaci�n.",
						InfoTraza.NivelTraza.error));
				ex.printStackTrace();
				error = true;
			}
	...
	public void arrancarRecurso(Integer indice) {
		...
		try {
			...
			logger.debug("GestorRecursos: Es necesario arrancar el recurso "+ nombre + ", recuperando interfaz de gesti�n.");
			trazas.aceptaNuevaTraza(new InfoTraza("GestorRecursos",
					"Es necesario arrancar el recurso "+ nombre + ", recuperando interfaz de gesti�n.",
					InfoTraza.NivelTraza.debug));
		...
		if (errorEnArranque) { // ha ocurrido alg�n problema en el arranque del
			...
			if (indice.intValue() == (this.nombresRecursosGestionados.size() - 1)) { // ya
																						...
																						// que
																						// arrancar
				logger.debug("GestorRecursos: Terminado proceso de arranque autom�tico de recursos.");
				trazas.aceptaNuevaTraza(new InfoTraza("GestorRecursos",
	...
	public void decidirTratamientoErrorIrrecuperable() {
		...
		logger.debug("GestorRecursos: Se decide cerrar el sistema ante un error cr�tico irrecuperable.");
		trazas.aceptaNuevaTraza(new InfoTraza("GestorRecursos",
	...
	public void finalizarRecurso() {
		...
		logger.debug("GestorRecursos: finalizarRecurso():Este m�todo no est� implementado");
		trazas.aceptaNuevaTraza(new InfoTraza("GestorRecursos",
				"finalizarRecurso():Este m�todo no est� implementado",
				InfoTraza.NivelTraza.debug));
	...
	public void crearRecurso() {
		// esto hay que recuperarlo de los par�metros
		logger.debug("GestorRecursos: crearRecurso():Este m�todo no est� implementado");
		trazas.aceptaNuevaTraza(new InfoTraza("GestorRecursos",
		...
		while (enumRecursos.hasMoreElements()) {
			try {
				...
				trazas.aceptaNuevaTraza(new InfoTraza("GestorRecursos",
						...
						InfoTraza.NivelTraza.debug));
				itfGesAg.termina();
				logger.debug("GestorRecursos: Orden de terminaci�n ha sido dada al recurso "+ nombre + ".");
				trazas.aceptaNuevaTraza(new InfoTraza("GestorRecursos",
						"Orden de terminaci�n ha sido dada al recurso "+ nombre + ".",
						InfoTraza.NivelTraza.debug));
			} catch (Exception ex) {
