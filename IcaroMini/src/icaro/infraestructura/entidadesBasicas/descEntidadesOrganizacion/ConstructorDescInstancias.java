	...
	public DescInstanciaAgenteAplicacion construirDescInstanciaAgenteAplicacion(
		...
		try {
			...
			if (encontrado) {
				...
				// obtener el nodo espec�fico
				Nodo nodo = instancia.getNodoEspecifico();
				if (nodo == null) {
					// obtener el nodo com�n de las instancias de agentes
					nodo = config.getDescOrganizacion().getInstancias()
							.getAgentesAplicacion().getNodoComun();
					if (nodo == null) {
						// obtener el nodo com�n de todas las instancias
						nodo = config.getDescOrganizacion().getInstancias()
								.getNodoComun();
					}
				}
			...
			e.printStackTrace();
			throw new UsoRecursoException(
					"Error al interpretar la descripci�n de la instancia de agente con id "
							+ id);
		}
	...
	public DescInstanciaGestor construirDescInstanciaGestor(String id)
		...
		try {
			...
			if (encontrado) {
				...
				Nodo nodo = instancia.getNodoEspecifico();
				if (nodo == null) {
					// obtener el nodo com�n de las instancias de gestores
					nodo = config.getDescOrganizacion().getInstancias()
				...
				throw new UsoRecursoException(
						"Error al interpretar la descripci�n de la instancia de gestor con id "
								+ id);
			return descGestor;
		} catch (Exception e) {
			e.printStackTrace();
	...
	public DescInstanciaRecursoAplicacion construirDescInstanciaRecursoAplicacion(
		...
		try {
			...
			if (encontrado) {
				...
				Nodo nodo = instancia.getNodoEspecifico();
				if (nodo == null) {
					// obtener el nodo com�n de las instancias de recursos de aplicaci�n
					nodo = config.getDescOrganizacion().getInstancias()
