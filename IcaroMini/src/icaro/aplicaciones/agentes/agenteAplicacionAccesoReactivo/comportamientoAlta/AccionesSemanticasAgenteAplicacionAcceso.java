...
// al automata para que transite y ejecute nuevas acciones
	...
	//Se muestra el formulario para rellenar con los datos del nuevo usuario
		...
		try {
            visualizacionAcceso.mostrarMensajeAviso("Acceso Incorrecto", "Procedemos a darle de Alta ");
			visualizacionAcceso.cerrarVisualizadorAcceso();
            ...
            try {
				...
				trazas.aceptaNuevaTraza(new InfoTraza(this.nombreAgente,"Ha habido un problema en mostrarAltaUsuario", InfoTraza.NivelTraza.error));
   ...
   //         itfUsoPropiadeEsteAgente.aceptaEvento(new EventoRecAgte("incorrecto",nombreAgente,nombreAgente));
            informaraMiAutomata("incorrecto", null);
            } catch (Exception exc) {
                Logger.getLogger(AccionesSemanticasAgenteAplicacionAccesoOld.class.getName()).log(Level.SEVERE, null, exc);
                informaraMiAutomata("error", null);
            }
...
/*
   ...
   //         trazas.aceptaNuevaTraza(new InfoTraza("AgenteAplicacionAlta",
	...
	public void mostrarNuevoUsuario (String username, String password){
		try{
			...
			logger.error("Excepcion en recurso de visualizaci�n para altas.");

				trazas.aceptaNuevaTraza(new InfoTraza(this.nombreAgente,
															  "Excepcion en recurso de visualizaci�n para altas.",
															  InfoTraza.NivelTraza.error));
			try{
 ...
 // Orden de cierre al  visualizador porque atendemos la peticion del usuario
   ...
   informaraOtroAgenteReactivo("peticion_terminar_todo", null, NombresPredefinidos.NOMBRE_GESTOR_AGENTES,itfUsoGestorAReportar);
        /*
        try {
       ...
       if (infoComplementaria == null) {
        ...
        try {
			...
			trazas.aceptaNuevaTraza(new InfoTraza(nombreAgente,
															  "Ha habido un problema enviar el evento con informacion "+ input + "a  a si mismo",
															  InfoTraza.NivelTraza.error));
			}

   }
 */
   public void informaraOtroAgenteReactivo(String input, Object[] infoComplementaria,String IdentAgenteReceptor, ItfUsoAgenteReactivo itfUsoAgenteReceptor){
      ...
      // En primer lugar se crea el evento con  la informacion de entrada
            ...
            if (infoComplementaria == null){
             eventoaEnviar = new EventoRecAgte(input, nombreAgente, IdentAgenteReceptor);
                 ...
                 {
               eventoaEnviar = new EventoRecAgte(input,infoComplementaria, nombreAgente, IdentAgenteReceptor);
                   }
